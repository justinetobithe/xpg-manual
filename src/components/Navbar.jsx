...
export default function Navbar({ title = "" }) {
    ...
    const ordered = useMemo(() => {
        ...
        const rest = languages.filter((l) => l.code !== "en").sort((a, b) =>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 grid grid-cols-3 items-center">
                <div className="flex items-center gap-3">
                    <Link to="/" className="inline-flex items-center">
                        <img
                            src="/logo_2.png"
                            alt={t("brand.name")}
                            className="h-8 w-auto md:h-10 hover:opacity-90 transition"
                        />
                    </Link>
                </div>
                ...
                <div className="justify-self-end">
                    <Menu as="div" className="relative inline-block text-left">
                        <Menu.Button className="inline-flex items-center gap-2 rounded-md border border-[#A66C13] px-3 py-1.5 text-sm md:text-base font-bold text-gray-100 hover:border-primary-400 transition">
                                        ...
                                        <Menu.Item key={l.code}>
                                            {({ active }) => (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm ${active ? "bg-orange-500/10" : ""
                                                        } ${l.code === current.code
                                                            ? "text-primary-300"
                                                            : "text-gray-100"
                                                        }`}
